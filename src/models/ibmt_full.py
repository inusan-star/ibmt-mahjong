...
class IBMTFull(nn.Module):
    """
    Intent-Belief Multi-task Transformer for Full Intent and Belief Prediction
    """
    ...
    def __init__(
        ...
        # IBMT_full: Intent head for yaku prediction
        self.intent_head = nn.Sequential(nn.Linear(emb_dim, emb_dim), nn.ReLU(), nn.Linear(emb_dim, config.NUM_YAKU))
        # IBMT_full: Belief head for 3 players' shanten number prediction
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        # IBMT_full: Intent prediction
        intent = self.intent_head(x_avg)

        # IBMT_full: Belief prediction
        ...
        return {"action": action, "intent": intent, "belief": belief}
