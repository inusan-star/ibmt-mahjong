...
class CNNObsEncoder(BaseObsEncoder):
    """CNN encoder."""

    def __init__(self):
        self._num_channels = 350
    ...
    def shape(self) -> tuple:
        return (self._num_channels, 34, 1)
    ...
    def _encode_draw(self, obs: mjx.Observation) -> np.ndarray:
        ...
        if last_event.type() == mjx.EventType.DRAW:
            tile = obs.draws()[-1]
            res[0, tile.type(), 0] = True

            if tile.is_red():
                res[1, tile.type(), 0] = True

        return res

    def _encode_melds(self, obs: mjx.Observation) -> np.ndarray:
        res = np.zeros((68, 34, 1), dtype=np.bool_)
            ...
            for meld_idx, tiles_count in enumerate(opened[player][:4]):
                ...
                for tile_type, count in enumerate(tiles_count):
                    for _ in range(count):
                        if tile_pos < 4:
                            res[meld_tile_offset + tile_pos, tile_type, 0] = True
                            tile_pos += 1

            for red_tile_type in player_red_tiles[player]:
                res[player_offset + 16, red_tile_type, 0] = True

        return res

    def _encode_discards(self, obs: mjx.Observation) -> np.ndarray:
        res = np.zeros((120, 34, 1), dtype=np.bool_)
    ...
    def _encode_kyotaku(self, obs: mjx.Observation) -> np.ndarray:
        res = np.zeros((4, 34, 1), dtype=np.bool_)
        kyotaku = obs.kyotaku()
        res[min(kyotaku, 3), :, 0] = True

        return res

    def _encode_turn(self, obs: mjx.Observation) -> np.ndarray:
        res = np.zeros((19, 34, 1), dtype=np.bool_)
        draw_count = 0
